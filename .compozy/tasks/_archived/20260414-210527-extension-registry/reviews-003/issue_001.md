...
- Decision: `valid`
- Root cause: The `extensions` table existed before commit `ec0bca8` added `registry_slug`, `registry_name`, and `remote_version`. `internal/store/globaldb/global_db.go` now creates those columns for new databases, but `internal/store/globaldb/migrate_workspace.go` still has no idempotent migration for legacy databases that already have an `extensions` table without them.
- Why this is a real defect: `Registry.List()`, `Registry.Get()`, and the install upsert in [internal/extension/registry.go](/Users/pedronauck/Dev/compozy/_worktrees/ext-registry/internal/extension/registry.go) all select or write the new provenance columns unconditionally, so upgraded databases can fail with `no such column` errors.
- Intended fix: Add a minimal legacy-schema migration in `internal/store/globaldb/migrate_workspace.go` and a regression test in `internal/store/globaldb/global_db_test.go`. This is the smallest root-cause fix even though it touches files outside the six primary code files.
- Resolution: Added `migrateExtensionColumns()` to `internal/store/globaldb/migrate_workspace.go` so legacy `extensions` tables are upgraded before schema ensure runs, even when the database predates the session-table migration path.
- Verification: Added a reopen-on-legacy-schema regression in `internal/store/globaldb/global_db_test.go`, ran `go test ./internal/store/globaldb`, and then `make verify`.
