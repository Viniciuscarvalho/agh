...
- Decision: `valid`
- Root cause: `installWithConfig()` always builds `ExtensionInfo{Enabled: true}` and the `ON CONFLICT(name) DO UPDATE` clause writes `enabled = excluded.enabled`, so `WithInstallReplaceExisting()` overwrites the persisted enabled state with `true`.
- Why this is a real defect: replacing a disabled extension silently re-enables it, which mutates user state unrelated to the package contents being refreshed.
- Intended fix: Preserve the existing row's enabled flag during the conflict update instead of writing the inserted value back, and add a regression test in `internal/extension/registry_test.go`.
- Resolution: Removed `enabled = excluded.enabled` from the replace-existing upsert in `internal/extension/registry.go`, so replacement updates keep the stored enabled flag.
- Verification: Added `TestRegistryInstallReplaceExistingPreservesEnabledState` in `internal/extension/registry_test.go`, ran `go test ./internal/extension`, and then `make verify`.
