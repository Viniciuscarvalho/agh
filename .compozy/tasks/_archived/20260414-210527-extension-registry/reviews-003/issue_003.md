...
- Decision: `valid`
- Root cause: the `replaceExisting` branch returns the raw `db.Exec` error, while the surrounding registry code consistently wraps persistence failures with operation context and the extension name.
- Why this is a real defect: callers lose the failing extension identity on replace/update failures, which makes operational debugging materially worse than the non-replace path.
- Intended fix: wrap the replace-existing persistence failure with `fmt.Errorf("extension: persist %q: %w", info.Name, err)` and cover it with a regression test that forces the write path to fail.
- Resolution: Wrapped the replace-existing write failure in `internal/extension/registry.go` with `extension: persist "<name>"`.
- Verification: Added `TestRegistryInstallReplaceExistingWrapsPersistErrors` in `internal/extension/registry_test.go`, ran `go test ./internal/extension`, and then `make verify`.
