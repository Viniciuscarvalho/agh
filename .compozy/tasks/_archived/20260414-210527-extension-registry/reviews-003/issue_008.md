...
- Decision: `valid`
- Root cause: the retry branch in `doRequest()` discards `response.Body.Close()` before sleeping and retrying, so cleanup failures are lost.
- Why this is a real defect: ignored close errors violate the repo rule even in retry paths, and the existing client logger already provides a safe place to surface them without changing retry control flow.
- Intended fix: close through the shared helper and log any close failure before the retry sleep, with a regression test that captures the log path.
- Resolution: The retry path now closes through `closeResponseBody(...)` and emits a debug log when that close fails before a retry.
- Verification: Added `TestClientRetriesHTTP500LogsCloseErrorBeforeRetry` in `internal/registry/github/client_test.go`, ran `go test ./internal/registry/github`, and then `make verify`.
