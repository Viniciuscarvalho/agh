...
In `finalizeStopped`, when `waitErr != nil`, the code calls `session.processHandle().Stderr()` at line 328 without a nil guard. `processHandle()` can return `nil` if the process was already cleared by a concurrent stop path or never fully initialized. A few lines later (line 346), the same pattern is used _with_ a nil guard (`if proc := session.processHandle(); proc != nil`), showing awareness of the problem. The first call lacks this guard and will panic.
