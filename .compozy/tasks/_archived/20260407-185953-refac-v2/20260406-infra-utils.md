...
This is expected and correct for the composition root. The daemon is _designed_ to import everything. The key question is whether it's _doing too much_ beyond wiring.
...
The packages are already well-organized. The real problem is not that they're small -- it's that `testutil` is _too_ small (missing helpers that are duplicated elsewhere) and that `daemon` is too large (contains domain logic that belongs elsewhere).
