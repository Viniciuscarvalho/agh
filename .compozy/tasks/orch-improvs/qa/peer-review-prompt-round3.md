...
REVIEW FOCUS:
- This is round 3 after two prior orchestration-hardening peer-review rounds were incorporated.
- Pressure-test the new aggregate + child-spec structure.
- Pay special attention to `TaskExecutionProfile`, guided coordinator mode, worker claim/session selection, per-task sandbox selection, review-gate routing, and the boundary that channels/profiles/skills/cursors are not authority.
- Treat review-gate and task-execution-profile additions as the primary changed scope since round 2.
