...
Because the fixture set contains just one extension and that one is reported healthy/registered, this test never proves that the fallback excludes unhealthy or unregistered extensions. Add a second installed extension that `runtime.Get()` reports as missing, disabled, or unregistered, then assert it does _not_ survive the partial-start recovery.
