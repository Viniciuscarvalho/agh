...
# Task 2: AGH\_\* Environment Variables and Remove BuildHookConfig from Interface
...
- [x] 2.1 Update `internal/kernel/api.go` `agentRuntimeEnv()` — replace all COLLAB*\*/AGI*\_ with AGH\_\_ equivalents
...
- [ADR-003: AGH\_\* Environment Variable Prefix Standardization](adrs/adr-003.md) — Defines the variable mapping
- [ADR-004: Remove BuildHookConfig from AgentDriver Interface](adrs/adr-004.md) — Defines the interface change

## Deliverables

- Updated env vars across all files (zero references to COLLAB*\* or AGI*\* remaining)
...
- Unit tests:
  - [x] `agentRuntimeEnv()` returns only AGH\_\* prefixed env vars
  ...
  - [x] No references to COLLAB*\* or AGI*\* exist in codebase (grep verification)
