...
- Unit tests:
  ...
  - [ ] `environment.ready` event fires after Prepare succeeds with instance ID
  ...
  - [ ] `environment.sync.after` event fires with sync stats
  - [ ] `environment.stop` event fires before sandbox teardown
  ...
  - [ ] Host API `environment/info` returns details for valid session
