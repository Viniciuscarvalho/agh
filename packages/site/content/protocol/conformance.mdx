...
<OperatorNote label="Implementation status">
  The current repository has package tests for the v0 AGH Runtime implementation, but it does not
...
- preserve envelope `reply_to`, `interaction_id`, `trace_id`, and `causation_id` when using NATS
  request-reply
