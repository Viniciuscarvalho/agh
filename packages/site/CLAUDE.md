...
```bash
cd packages/site && bun run source:generate   # Fumadocs MDX → .source/  (runs before typecheck/test/build)
cd packages/site && bun run content:generate  # Velite MDX/YAML → .velite/  (runs after source:generate)
