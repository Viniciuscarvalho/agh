		...
		httpClient: &http.Client{
			Transport: roundTripperFunc(func(req *http.Request) (*http.Response, error) {
				...
				case req.Method == http.MethodPost && req.URL.Path == "/api/network/send":
					...
					return newHTTPResponse(
						http.StatusOK,
						`{"message":{"id":"msg-1","session_id":"sess-a","channel":"builders","kind":"say","ext":{"agh.workflow_id":"wf-1","agh.handoff_version":3}}}`,
					), nil
				case req.Method == http.MethodGet && req.URL.Path == "/api/network/inbox":
					...
					return newHTTPResponse(
						http.StatusOK,
						`{"messages":[{"protocol":"agh-network/v0","id":"msg-inbox","kind":"direct","channel":"builders","from":"reviewer.sess-a","ts":1775823000,"body":{"text":"review this","intent":"review"},"ext":{"agh.workflow_id":"wf-1","agh.handoff_version":3}}]}`,
					), nil
				default:
