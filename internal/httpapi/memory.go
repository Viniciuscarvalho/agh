...
func (h *Handlers) writeMemory(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		respondError(c, http.StatusBadRequest, fmt.Errorf("httpapi: decode memory write request: %w", err))
		return
	}
...
func (h *Handlers) consolidateMemory(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil && !errors.Is(err, io.EOF) {
		respondError(c, http.StatusBadRequest, fmt.Errorf("httpapi: decode memory consolidate request: %w", err))
		return
	}
